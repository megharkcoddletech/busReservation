...
const registerUser = async (req, res) => {
    ...
    try {
        const { name, username, password, email, age, contactNumber, gender } = req.body;
        const signUp = await userDb.createUser(name, username, password, email, age, contactNumber, gender);
        if(signUp.length>0){
            res.status(404).json({success: "false", message:"user already exists"})
        }
        if (  name === undefined || password === undefined || username === undefined || email === undefined || age === undefined || contactNumber === undefined || gender === undefined) {
...
const checkLogin = async (req, res) => {
   try {
     ...
     if (checkLoginQuery.length > 0) {
         ...
         if (checkLoginQuery[0].username === body.userame && checkLoginQuery[0].password === body.password) {
